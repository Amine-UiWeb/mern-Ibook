...
const WorkPage = () => {
  ...
  // pathname's structure: /works/:workId
  ...
  const { data: authorKeyData } = useFetchData({ 
    end: 'b_authorByKey', dep: searchData, pathname: pathname 
  })
  ...
  const { data: authorNameData } = useFetchData({ 
    end: 'b_authorByName', dep: searchData, pathname: pathname 
  })
  ...
  return (
    <div className="work-details">
      ...
      <h1 className="h1 title ta-c p-0">{ title || <TitleSkeleton /> }</h1>
      ...
      { authorSearchData && (
          <section className="author-brief">
            ...
            <div className="flex-row ai-fs flex-wrap mt-1">

              <div className="min-w-fit p-in-1 ta-c">
      ...
      { (!workFetchComplete || authorWorks?.docs?.length > 0) && (
          ...
          <section className="author-works">
            <h2 className="h3 mb-0-5">Other works by <b>{authorName || 'author'}</b>:</h2>
